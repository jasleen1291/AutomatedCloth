...
public class panel6 extends JPanel {
    ...
    public JCheckBox[] color = new JCheckBox[5];
    JLabel bb;
    JButton submit;
    String cost;
    ...
    panel6() {
        int e = 0;
        setVisible(false);
        JLabel bg = new JLabel(new ImageIcon(getClass().getResource("/images/bb.jpg")));
        bg.setLayout(null);
        for (int q = 0; q <= 4; q++) {
                ...
                for (int q = 0; q <= 4; q++) {
                    if (color[q].isSelected()) {
                        try {
                            ...
                            Query = "select id from items where category like'babyboy'&& color like '" + col[q] + "' ";
